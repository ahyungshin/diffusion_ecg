...
def parse_id(id_, missing_ratio=0.1):
    data = pd.read_csv("/root/agilab22/4Diffusion/data/physio/set-a/{}.txt".format(id_))
    # set hour
...
def get_idlist():
    patient_id = []
    for filename in os.listdir("/root/agilab22/4Diffusion/data/physio/set-a"):
...
class Physio_Dataset(Dataset):
    def __init__(self, eval_length=48, use_index_list=None, missing_ratio=0.0, seed=0):
        ...
        self.gt_masks = []
        path = (
            "/root/agilab22/4Diffusion/data/physio_missing" + str(missing_ratio) + "_seed" + str(seed) + ".pk"
        )
